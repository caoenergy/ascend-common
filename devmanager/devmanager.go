...
func GetDeviceManager(resetTimeout int) (*DeviceManager, error) {
	devManagerOnce.Do(func() {
		...
		fmt.Printf("get card list from dcmi reset timeout is %d\n", resetTimeout)
		for currentTime, retryCount := 0, 0; currentTime <= resetTimeout; currentTime += int(retryDelay) {
			if err := dcMgr.DcInit(); err != nil {
				fmt.Printf("deviceManager init failed, prepare dcmi failed, err: %v\n", err)
				return
			}
			...
			retryCount++
			fmt.Printf("deviceManager get card list failed (attempt %d), cardNum=%d, cardList=%v, err: %v\n", retryCount, cardNum, cardList, err)
			if currentTime+int(retryDelay) <= resetTimeout {
				if err = dcMgr.DcShutDown(); err != nil {
					fmt.Printf("deviceManager shut down failed, err: %v\n", err)
					return
				}
...
func AutoInit(dType string, resetTimeout int) (*DeviceManager, error) {
	...
	switch devType {
	case common.Ascend910A, common.Ascend910B, common.Ascend910A3:
