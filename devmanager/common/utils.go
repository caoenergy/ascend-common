...
var (
	reg910A = regexp.MustCompile(Ascend910APattern)
	reg910B = regexp.MustCompile(Ascend910BPattern)
	reg310P = regexp.MustCompile(Ascend310PPattern)
)
...
func GetDeviceTypeByChipName(chipName string) string {
	...
	if strings.Contains(chipName, Ascend310BNo) {
		return Ascend310B
	}
	if strings.Contains(chipName, Ascend310No) {
		return Ascend310
	}
